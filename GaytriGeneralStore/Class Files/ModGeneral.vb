...
#Region "Function"
    ...
    Public Sub M_GetClosingBalance_New(ByVal ledgerId As Integer, ledgerType As String, gId As Integer, startDate As DateTime, endDate As DateTime)
        ...
        obj.AddCmdParameter("@YearID", Dtype.int, FrmMDIMain.cmbFinYr.SelectedValue, ParaDirection.Input, True)
...
#Region "Methods"
        ...
        For i As Integer = 0 To M_dsSettings.Tables(0).Rows.Count - 1
            Select Case M_dsSettings.Tables(0).Rows(i)("SettingName")
                ...
                Case "Whatsapp Instance Id"
                    ...
                    Exit Select
                Case "Website Origin For Whatsapp"
                    M_WhatsappWebsiteOrigin = M_dsSettings.Tables(0).Rows(i)("SettingValue")
                    Exit Select
                ...
                Case "Delivery: Deliverd Item Report File Name"
                    ...
                    Exit Select
                Case "Move To Sales Register After Ever Bill"
                    M_MoveToSalesRegisterAfterEverBill = M_dsSettings.Tables(0).Rows(i)("SettingValue")
                    Exit Select
                Case "WhatsApp Provider Name"
                    M_WhatsAppProviderName = M_dsSettings.Tables(0).Rows(i)("SettingValue")
                    Exit Select
            End Select
        Next
    ...
    Public Sub sendWhatsApp_SendText(ByVal toMoNum As String, ByVal msgTxt As String, ByVal countryCode As String, ByVal CCWhatsapp As String)
        ...
        Select Case UCase(M_WhatsAppProviderName)
            Case "Interakt"

                Exit Select
            Case "11ZA"
                send11ZAWhatsApp_SendText(toMoNum, M_TemplateName, M_WhatsAppmsgBody, countryCode)
                Exit Select
            Case "REGULAR", "", "-"
    ...
    Public Sub sendWhatsApp_SendFile(ByVal toMoNum As String, ByVal filePath As String, ByVal countryCode As String)
        ...
        Select Case UCase(M_WhatsAppProviderName)
            Case "Interakt"
                ...
                Exit Select
            Case "11ZA"
                send11ZAWhatsApp_SendFile(toMoNum, filePath, M_TemplateName, countryCode)
                Exit Select
            Case "REGULAR", "", "-"
                'Create Instance On Application Startup:-
                Dim ClsNotificationApp_I As ClsNotificationApp
    ...
    Public Sub sendWhatsApp_SendFileWithText(ByVal toMoNum As String, ByVal filePath As String, ByVal captionText As String, ByVal countryCode As String)
        ...
        Select Case UCase(M_WhatsAppProviderName)
            Case "Interakt"
                ...
                Exit Select
            Case "11ZA"
                send11ZAWhatsApp_SendFileWithText(toMoNum, filePath, M_TemplateName, M_WhatsAppmsgBody, countryCode)
                Exit Select
            Case "REGULAR", "", "-"
                'Create Instance On Application Startup:-
                Dim ClsNotificationApp_I As ClsNotificationApp
    ...
    Public Function GetWhatsappTemplateName(ByVal tmpmsg As String) As String
        Dim result = (From row In dsMiscMaster.Tables(0).AsEnumerable()
                      Where String.Equals(row("MiscName").ToString().Trim(), tmpmsg.Trim(), StringComparison.OrdinalIgnoreCase) AndAlso
                            CBool(row("IsActive")) = True AndAlso
                            CInt(row("CId")) = M_CId AndAlso
                          Trim(row("Data1")) <> ""
                      Select row("Data1").ToString().Trim()
             ).FirstOrDefault()

        Return result
    End Function

    Public Sub send11ZAWhatsApp_SendFile(ByVal toMoNum As String, ByVal filePath As String, ByVal templatename As String, ByVal countryCode As String)
        If M_TemplateName = "" Then
            MsgBox("Template Name Not found", MsgBoxStyle.Information)
            Exit Sub
        End If

        'Upload File > Hostinger
        Dim filelink As String
        If File.Exists(filePath) Then
            filelink = UploadInvoiceToHostinger(filePath)
        Else
        ...
        Try
            ...
            Dim client As New RestClient("https://api.11za.in/apis/template/sendTemplate")
            Dim request As New RestRequest(Method.POST)

            request.AlwaysMultipartFormData = True

            request.AddParameter("authToken", M_WhatsappInstanceId)
            request.AddParameter("name", M_TemplateName)
            request.AddParameter("sendto", countryCode & toMoNum)
            request.AddParameter("originWebsite", M_WhatsappWebsiteOrigin)
            request.AddParameter("language", "en")
            request.AddParameter("templateName", templatename)
            If filelink.Contains("http") Then
                request.AddParameter("myfile", filelink)
            End If

            Dim response As IRestResponse = client.Execute(request)
            If response.StatusCode <> Net.HttpStatusCode.OK Then
                MsgBox("Error: " & response.StatusCode.ToString() & vbCrLf & response.Content)
            End If
        Catch ex As Exception
            MsgBox(ex.Message)
        End Try
    ...
    Public Sub send11ZAWhatsApp_SendFileWithText(ByVal toMoNum As String, ByVal filePath As String, ByVal templatename As String, ByVal tmpdata As List(Of String), ByVal countryCode As String)
        If M_TemplateName = "" Then
            MsgBox("Template Name Not found", MsgBoxStyle.Information)
            Exit Sub
        End If
        ...
        Try
            ...
            For i As Integer = 0 To tmpdata.Count - 1
                request.AddParameter("data[" & i & "]", tmpdata(i))
            Next
    ...
    Public Sub send11ZAWhatsApp_SendText(ByVal toMoNum As String, ByVal templatename As String, ByVal tmpdata As List(Of String), ByVal countryCode As String)
        If M_TemplateName = "" Then
            MsgBox("Template Name Not found", MsgBoxStyle.Information)
            Exit Sub
        End If
    ...
    Public Function UploadInvoiceToHostinger(localFilePath As String) As String
        Try
            Dim ftpHost As String = "ftp://195.35.44.38"
            Dim ftpUser As String = "u354894311.sunrisesoftware"
            Dim ftpPass As String = "FtpPwd@2024"
            Dim foldpath As String = "/invoices/" & Path.GetFileName(localFilePath)

            Dim ftpUrl As String = ftpHost & foldpath
            ...
            request.EnableSsl = False ' Hostinger normal FTP
            ...
            ' Public URL to send in WhatsApp
            Return "https://sunrisesoftware.in" & foldpath

        Catch ex As Exception
            MsgBox("FTP Error: " & ex.Message)
            Return Nothing
        End Try
    End Function
    ...
    Public Sub GridToDataset(ByVal ds As DataSet, ByVal dt As DataTable)
        ds.Clear()
        ds.Reset()
        dt.Reset()

        For Each column As DataGridViewColumn In frmGridToExcel.grdData.Columns
            dt.Columns.Add(column.HeaderText)
        Next

        For Each row As DataGridViewRow In frmGridToExcel.grdData.Rows
